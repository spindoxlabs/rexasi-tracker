...
def drow_detection_to_kitti_string(dets_xy, dets_cls, occluded):
    ...
    if occluded is None:
        occluded = np.zeros(len(dets_xy), dtype=np.int_)
...
def kitti_string_to_drow_detection(s):
    ...
    occluded = np.array(occluded, dtype=np.int_)
...
def get_precision_recall_one_hot(
    ...
    if dist_bins is not None:
        gt_hist = np.zeros(len(dist_bins), dtype=np.int_)
        tp_hist = np.zeros(len(dist_bins), dtype=np.int_)
        fp_hist = np.zeros(len(dist_bins), dtype=np.int_)
